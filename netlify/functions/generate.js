...
exports.handler = async (event) => {
  ...
  try {
    ({ keywords, style, randomness, mode, name } = JSON.parse(event.body || '{}'));
  } catch {
  ...
  if (mode === 'analyse') {
    if (!name || !name.trim()) {
      return { statusCode: 400, body: JSON.stringify({ error: 'name обязателен' }) };
    }
    ...
    if (!aResp.ok) {
      ...
      return { statusCode: aResp.status, body: JSON.stringify({ error: `API ${aResp.status}: ${t.slice(0,200)}` }) };
    }
    ...
    catch { return { statusCode: 500, body: JSON.stringify({ error: 'Невалидный JSON от AI', raw: aRaw.slice(0,200) }) }; }
