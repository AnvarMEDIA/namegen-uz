...
export const RANDOMNESS_KEYS = ['low', 'medium', 'high'] as const;
...
export function isRandomnessKey(value: unknown): value is RandomnessKey {
  return typeof value === 'string' && RANDOMNESS_KEY_SET.has(value);
}
...
export const STYLE_LABELS: Readonly<Record<StyleKey, string>> = {
  ...
  brandable:
    'Invented/coined words — sounds real but means nothing (Spotify, Kodak, Zillow, Canva, Google)',
  evocative:
    'Evocative names — strong emotional or conceptual association (RedBull, Amazon, Patagonia, Forever21)',
  compound:
    'Compound words — two meaningful words fused or blended (FedEx, Microsoft, Facebook, Snapchat)',
  alternate:
    'Alternate spelling — intentional creative misspelling (Lyft, Fiverr, Tumblr, Flickr, Dribbble)',
  nonEnglish:
    'Non-English words — foreign language roots sounding premium and global (Toyota, Audi, Volvo, Zara)',
  real_words:
    'Real words — common English words used unexpectedly as brand names (Apple, Amazon, Stripe, Notion)',
  uzbek_roots:
    'Uzbek & Turkic roots — authentic Central Asian roots with modern feel (Nurli, Baxtzor)',
};

export const STYLE_RULES: Readonly<Record<StyleKey, string>> = {
  auto:
...
export const RANDOMNESS: Readonly<Record<RandomnessKey, RandomnessSpec>> = {
  low: {
    temperature: 0.3,
    hint:
