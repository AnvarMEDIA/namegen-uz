// Curated Uzbek inspiration corpus for AI brand naming.
//
// Contains 229 semantic anchors (length 2-10) used as inspiration
// seeds — final brand names generated by AI typically range 4-8 chars
// but may extend up to 12 for compound forms.
//
// Curated by a native Uzbek speaker. Used by lib/prompts/generate.ts
// (Step 2) — the AI receives 10–15 sampled roots per request and is
// free to riff on them when proposing brand names. Short roots
// (e.g. "nur", "gul", "aql") and long ones (e.g. "samarqand",
// "kashfiyot") are intentional inspiration anchors, not direct
// candidates: the existing phonetic + length rules in
// lib/prompts/styles.ts still constrain the FINAL brand-name shape
// (4..12 chars, /^[a-z0-9]+$/, etc.).
//
...
//     `'` (e.g. "yo'l", "tog'", "ma'no", "g'ayrat") — that's a
//     legitimate Uzbek letter (modifier marking the soft consonant).
//     Charset for the latin field is /^[a-z'ʻ`]+$/.
